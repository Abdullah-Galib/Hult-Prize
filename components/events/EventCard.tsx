...
export default function EventCard({ event }: EventCardProps) {
  // Robust date parsing to prevent UI overflow
  const d = new Date(event.date);
  const isValidDate = !isNaN(d.getTime());
  
  const day = isValidDate ? d.getDate().toString().padStart(2, '0') : event.date.substring(0, 2);
  const month = isValidDate ? d.toLocaleString('en-US', { month: 'short' }).toUpperCase() : event.date.substring(3, 6).toUpperCase();
  ...
  return (
    <div className="bg-white dark:bg-[#131B2F] rounded-2xl overflow-hidden shadow-lg border border-slate-100 dark:border-white/10 group flex flex-col h-full transition-all hover:-translate-y-1 hover:shadow-2xl">
      <div className="relative h-48 bg-slate-200 dark:bg-slate-800 w-full overflow-hidden">
        ...
        {/* Adjusted width, text size and overflow hidden to prevent breaking */}
        <div className="absolute bottom-0 left-4 translate-y-1/2 bg-[#E6007F] text-white flex flex-col items-center justify-center w-14 h-14 rounded-lg shadow-md z-10 overflow-hidden">
